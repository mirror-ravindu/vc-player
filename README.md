...
[![Deploy on Railway](https://railway.app/button.svg)](https://railway.app/new/template?template=https%3A%2F%2Fgithub.com%2Fmirror-ravindu%2Fvc-player%2Ftree%2Fmaster&envs=API_ID%2CAPI_HASH%2CBOT_TOKEN%2CSESSION_STRING%2CCHAT%2CLOG_GROUP%2CADMINS%2CADMIN_ONLY%2CMAXIMUM_DURATION%2CSTREAM_URL%2CREPLY_MESSAGE&optionalEnvs=LOG_GROUP%2CADMIN_ONLY%2CMAXIMUM_DURATION%2CSTREAM_URL%2CREPLY_MESSAGE&API_IDDesc=api_id+part+of+your+Telegram+API+Key+from+my.telegram.org%2Fapps&API_HASHDesc=api_hash+part+of+your+Telegram+API+Key+from+my.telegram.org%2Fapps&BOT_TOKENDesc=Bot+token+of+Bot%2C+get+from+%40Botfather&SESSION_STRINGDesc=Session+string%2C+read+the+README+to+learn+how+to+export+it+with+Pyrogram&CHATDesc=ID+of+Channel+or+Group+where+the+Bot+plays+Music&LOG_GROUPDesc=ID+of+the+group+to+send+playlist+If+CHAT+is+a+Group%2C+if+channel+thenleave+blank&ADMINSDesc=ID+of+Users+who+can+use+Admin+commands%28for+multiple+users+seperated+by+space%29&ADMIN_ONLYDesc=Change+it+to+%27N%27+if+you+want+to+make+%2Fplay+and+%2Fdplay+available+for+everyone.+By+default+only+admins+of+CHAT+can+use+it.&MAXIMUM_DURATIONDesc=Maximum+duration+of+song+to+be+played+using+%2Fplay+or+%2Fdplay&STREAM_URLDesc=URL+of+Radio+station+or+Youtube+live+video+url+to+stream+with+%2Fradio+command&REPLY_MESSAGEDesc=A+reply+message+to+those+who+message+the+USER+account+in+PM.+Make+it+blank+if+you+do+not+need+this+feature&ADMIN_ONLYDefault=Y&MAXIMUM_DURATIONDefault=1440)
